...
[HarmonyPatch(typeof(UnityXRHelper), "GetRootPositionOffsetForLegacyNodePose", typeof(UnityXRHelper.VRControllerManufacturerName))]
...
[HarmonyPatch(typeof(UnityXRHelper), "TryGetLegacyPoseOffsetForNode",
...
internal class UnityXRHelperTryGetLegacyPoseOffsetForNode
{
    ...
    private static void Postfix(UnityXRHelper.VRControllerManufacturerName manufacturerName, ref Vector3 position, ref Vector3 rotation, ref bool __result)
    {
        ...
        {
            __result = true;
            // it is the same as the HTC values but in case they decide to change it
